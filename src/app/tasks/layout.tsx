...
function TasksSidebarContent() {
  ...
  const selectedTaskId = searchParams.get("id") ? parseInt(searchParams.get("id")!) : null;

  const [searchTerm, setSearchTerm] = useState("");
  const [taskTagsMap, setTaskTagsMap] = useState<
    Map<number, { id?: number; name: string }[]>
  >(new Map());

  const [filterEstimationStatus, setFilterEstimationStatus] =
    useLocalStorage<string>("tasks-filter-estimation-status", "");
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter((task) => {
      ...
      const matchesTaskStatus =
        !filterTaskStatus || task.status === filterTaskStatus;
      ...
      const matchesTag =
        ...
        (task.id &&
          ...
          taskTagsMap
            .get(task.id)!
            .some((tag) => tag.id?.toString() === filterTag));

      return (
        matchesSearch &&
        matchesEstimationStatus &&
        matchesTaskStatus &&
        matchesTag
      );
    });
    ...
    <aside className="w-80 lg:w-96 border-r border-border bg-card flex flex-col flex-shrink-0">
      ...
      <div className="flex-1 overflow-auto scrollbar-thin p-3 space-y-1.5">
        ...
        {filteredTasks.map((task) => (
          ...
          >
            <div className="font-semibold text-sm mb-0.5 leading-snug">
              {task.name}
            </div>
...
export default function TasksLayout({
  ...
  return (
    <div className="h-full flex">
      <Suspense fallback={<div className="w-80 lg:w-96 border-r border-border bg-card" />}>
        <TasksSidebarContent />
      </Suspense>

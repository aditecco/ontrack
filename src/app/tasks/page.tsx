...
import { formatTime, formatDecimalHours, formatCurrency, formatDate, cn } from '@/lib/utils'
import { Plus, Search, Trash2, Edit, Download, AlertTriangle } from 'lucide-react'
...
export default function TasksPage() {
  const { tasks, fetchTasks, setSelectedTask, selectedTaskId, deleteTask } = useTaskStore()
        ...
        <div className="flex-1 overflow-auto scrollbar-thin p-4 space-y-2">
          {filteredTasks.map((task) => (
            ...
            >
              ...
              <div className="text-sm text-muted-foreground mb-2">{task.customer}</div>
          ...
          <div className="space-y-6">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">{selectedTask.customer}</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Delete "${selectedTask.name}"? This will also delete all time entries.`)) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className={cn(
                ...
                taskStats.remaining < 0 ? "border-destructive/50 bg-destructive/5" : "border-border"
              )}>
                <div className="flex items-center gap-2 mb-1">
                  <div className="text-sm text-muted-foreground">Estimated</div>
                  {taskStats.remaining < 0 && <AlertTriangle className="w-4 h-4 text-destructive" />}
                </div>
                <div className="text-2xl font-bold">{selectedTask.estimatedHours}h</div>
                <div className={cn(
                  "text-xs mt-1",
                  taskStats.remaining < 0 ? "text-destructive font-medium" : "text-muted-foreground"
                )}>
                  {taskStats.remaining >= 0 ? `${taskStats.remaining.toFixed(1)}h remaining` : `${Math.abs(taskStats.remaining).toFixed(1)}h over budget`}
                </div>
              </div>
              ...
              <div className="bg-card border border-border rounded-lg p-4">
                ...
                <div className="text-2xl font-bold">
                  {selectedTask.budget ? formatCurrency(selectedTask.budget) : 'N/A'}
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  {taskStats.entryCount} {taskStats.entryCount === 1 ? 'entry' : 'entries'}
                </div>
              </div>
            </div>
              ...
              {taskStats.dailyData.length > 0 ? (
                <div className="space-y-4">
                  {taskStats.dailyData.map((day, index) => {
                    const maxHours = Math.max(...taskStats.dailyData.map(d => d.hours), selectedTask.estimatedHours || 8)
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-sm font-bold">{day.hours.toFixed(2)}h</span>
                        </div>
